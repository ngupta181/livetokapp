...
class CoinController extends Controller
{
	...
	function addUpdateGift(Request $request)
	{
		...
		$data['animation_style'] = $animation_style;
		...
		if (!empty($Items)) {
			foreach ($Items as $rows) {
				...
				$edit = '<a data-toggle="modal" data-target="#giftsModal" data-id="' . $rows->id . '"
					data-price="' . $rows->coin_price . '" data-image="' . $rows->image . '" data-animation-style="' . $rows->animation_style . '" class="settings updateGift" title="Edit Gift" data-toggle="tooltip" data-original-title="Edit Gift" ' . $disabled . '><i class="i-cl-3 fas fa-edit col-blue font-20 pointer p-l-5 p-r-5"></i></a>';
